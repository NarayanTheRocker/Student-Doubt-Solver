...
# 2. Image Analysis Model (BLIP - OpenVINO Optimized)
# !!! IMPORTANT: This should be the directory containing your blip_caption.xml and blip_caption.bin files !!!
BLIP_OV_MODEL_DIR = "blip_ov" # <-- CHANGE THIS to your OpenVINO model directory
...
device = "cpu" # Kept for consistency, though OpenVINO manages its own devices
...
# Load BLIP models (Processor from HuggingFace, Model from OpenVINO)
try:
    print("Loading BLIP processor from HuggingFace...")
    # Load processor from Hugging Face Hub as per the optimized code snippet
    ...
    if os.path.isfile(BLIP_MODEL_XML) and os.path.isfile(BLIP_MODEL_BIN):
        ...
        print("✅ OpenVINO BLIP model loaded and compiled successfully.")
    else:
        print(f"❌ ERROR: OpenVINO model files not found.")
        print(f"    Checked for XML at: '{BLIP_MODEL_XML}'")
        print(f"    Checked for BIN at: '{BLIP_MODEL_BIN}'")
except Exception as e:
    print(f"❌ ERROR loading BLIP components: {e}")
...
def get_image_description(image_path):
    """Uses the OpenVINO-optimized BLIP model to generate a description of the image."""
    ...
    try:
        ...
        # Process inputs for conditional captioning as shown in the optimized example.
        # This provides a starting context ("a photo of") for the model.
        inputs = blip_processor(images=image, text=["a photo of"], return_tensors="np")

        # Map input names dynamically to the compiled model's expected inputs.
        # This assumes the model was exported with inputs in the order:
        # 1. input_ids, 2. attention_mask, 3. pixel_values
        ...
        # The output of the compiled model is a dictionary. The first value should be the logits.
        # We perform greedy decoding by taking the argmax along the vocabulary axis.
        ...
        print(f"Error during OpenVINO image description: {e}")
        ...
        if 'feedback' in request.form:
            ...
            if input_type == 'text':
                ...
                if image_file and image_file.filename != '':
                    ...
                    print("Analyzing image with OpenVINO BLIP...")
...
if __name__ == '__main__':
    # Updated check to look for the compiled OpenVINO model
